...
public class EmployeeDao implements EmpDaoInterface<Employee> {
	...
	public boolean saveData(Employee ob) {
		try {
			...
			PreparedStatement pstmt=con.prepareStatement("insert into emp_data(emp_id,emp_email,emp_name,emp_phone,emp_gender,emp_passowrd)"
					+ "values(?,?,?,?,?,?)");
			pstmt.setInt(1, ob.getEmpId());
			pstmt.setString(2, ob.getEmail());
			pstmt.setString(3, ob.getEmpName());
			pstmt.setString(4, ob.getEmpPhone());
			pstmt.setString(5, ob.getGender());
			pstmt.setString(6, ob.getPassowrd());
			pstmt.executeUpdate();
			return true;
		} catch (Exception e) {
			e.printStackTrace();
